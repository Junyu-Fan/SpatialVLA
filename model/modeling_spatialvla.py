...
# =============================================================================
# [模块总览]
# 这个文件实现 SpatialVLA 的核心前向与生成逻辑，主要由 4 层组成：
# 1) 几何编码层：Ego3DPositionEmbeddingMLP + backproject_patch
# 2) 视觉特征层：vision_tower + (可选) ZoeDepth + 3D位置融合
# 3) 跨模态对齐层：SpatialVLAMultiModalProjector（视觉到文本隐空间）
# 4) 语言动作层：Gemma2ForCausalLM 自回归生成动作 token
#
# [训练入口调用链]
# train/spatialvla_pretrain.py / train/spatialvla_finetune.py
#   -> Trainer.compute_loss (train/monkey_patch.py)
#   -> model(**inputs) -> SpatialVLAForConditionalGeneration.forward
#
# [推理入口调用链]
# test/test_huggingface.py
#   -> model.predict_action(model_inputs)
#   -> generate(...) -> prepare_inputs_for_generation -> forward
# =============================================================================
...
class Ego3DPositionEmbeddingMLP(nn.Module): #输入：每个视觉patch对应的3D点（或拼接后的多点坐标）。输出：与视觉token同维度的位置特征。目的：把几何信息注入视觉token（不是只靠RGB外观）
    ...
    def __init__(self, in_channels=3, num_pos_feats=768, n_freqs=8, logscale=True):
        ...
        # 输入 xyz 维度 -> 频率编码后维度：in_channels * (2*n_freqs + 1)
    ...
    def forward(self, xyz):
        """
        输入: xyz, shape=(B, N, C) 其中 C=3 或 3*子采样点数
        输出: position_embedding, shape=(B, N, num_pos_feats)
        调用方: SpatialVLAForConditionalGeneration.get_image_features
        """
...
class SpatialVLAMultiModalProjector(nn.Module): #一个MLP 把视觉token映射到文本token所在嵌入空间
    ...
    def forward(self, image_features):
        # 输入: (B, N_img, vision_hidden) -> 输出: (B, N_img, projection_dim)
...
class SpatialVLAForConditionalGeneration(SpatialVLAPreTrainedModel, GenerationMixin):
    def __init__(self, config: SpatialVLAConfig, vision_model=None, vision_zoe_model=None, projector_model=None, language_model=None):
        ...
        # [A] 视觉塔（默认 SigLIP）
        # 输入: 归一化 RGB (B,3,H,W)
        # 输出: patch 特征 (B,N_img,vision_hidden)
        ...
        # [C] 语言模型（Gemma2）
        # 输入: inputs_embeds (文本+图像替换后的序列)
        # 输出: logits (B,seq,vocab)
        ...
        if config.use_vision_zoe:
            # [D] 几何分支：ZoeDepth + 3D位置编码
            # 目标: 让每个图像 patch 注入几何位置信息
        ...
        if config.use_spatial_token:
            # 训练时将 action token 的输入 embedding 重映射到可学习空间向量
    ...
    def backproject_patch(self, K: torch.Tensor, depth: torch.Tensor, patch_size=14, reso=2) -> torch.Tensor:
        """
                ...
                输出:
                    patch_p_cam: patch级3D点, shape=(B,N_img,3*reso*reso)
                调用方:
                    get_image_features() 内部调用
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, intrinsic: torch.FloatTensor):
        """
        输入:
            pixel_values: RGB 图像, (B,3,H,W)
            intrinsic: 相机内参, (B,3,3)
        输出:
            image_features: 融合后视觉token, (B,N_img,text_hidden)
        调用方:
            forward() 在 pixel_values 非空时调用
        """
        ...
        if self.config.use_vision_zoe:
            ...
            # 关键融合: 2D语义特征 + 3D位置嵌入（同shape逐元素相加）
    ...
    def forward(
        ...
        [主训练/推理前向]
        主要输入:
            input_ids: 文本+动作 token 序列 (B,seq)
            pixel_values: 图像 (B,3,H,W)
            intrinsic: 相机内参 (B,3,3)
            labels: 训练标签 (B,seq)
        主要输出:
            logits: (B,seq,vocab)
            loss: 仅训练时可用
        外部调用:
            - 训练: Trainer.compute_loss -> model(**inputs)
            - 推理: generate 过程中反复调用
        """
        ...
        if self.config.use_spatial_token:
            ...
            # 将动作 token 的默认词嵌入替换成专用 spatial embedding
        ...
        if pixel_values is not None:
            ...
            # 用图像特征替换序列中 <image> 占位 token 的 embedding
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        """
        generate() 每一步解码前都会走这里。
        关键策略:
        - 仅第一步喂 pixel_values（避免重复编码图像）
        - position_ids +1（与 PaliGemma 位置索引对齐）
        """
        ...
        if cache_position[0] == 0:
            # 仅首 token step 传图像，后续 step 复用 KV cache
    ...
    def predict_action(
        ...
        """
        推理快捷接口（被 test/test_huggingface.py 调用）
        输入: processor 产出的 model_inputs
        输出: 仅新增生成片段（去掉输入前缀）
        """
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # 自定义加载后处理：当启用 spatial token 时，将末尾词嵌入与 spatial embedding 同步
